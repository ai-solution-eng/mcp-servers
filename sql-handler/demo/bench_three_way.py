...
SH_NFS = "http://sqlhandler-nfs.sqlhandler.svc.cluster.local:9097/mcp"
...
def bench_legs(clients, results):
    for label, client, tool, argof, pick in clients:
        out = {"leg": label}
        lat_w, lat_c = [], []
...
def main():
    results = {"started": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
               "mode": "three-way MCP: sqlhandler-nfs (NFS backend) vs sqlhandler (S3/MinIO) "
                       "vs ezpresto (PVC mirror); all ClusterIP"}
    sh_s3 = Mcp(SH, api_key=SH_KEY)
    sh_nfs = Mcp(SH_NFS, api_key=SH_KEY)
    ezp = Mcp(EZP, bearer=EZP_BEARER)

    parity = {}
    for name, sq, eq in PARITY:
        _, t_nfs = sh_nfs.call("run_sql", {"sql": sq, "output_format": "json"})
        _, t_s3 = sh_s3.call("run_sql", {"sql": sq, "output_format": "json"})
        _, t_ez = ezp.call("execute_query", {"query": eq})
        a, b, c = count_from(t_nfs), count_from(t_s3), count_from(t_ez)
        parity[name] = {"sh_nfs": a, "sh_s3": b, "ezpresto": c,
                        "equal": a == b == c}
        print("parity|%s|nfs=%s|s3=%s|ezp=%s|%s" % (name, a, b, c,
                                                    "EQUAL" if a == b == c else "MISMATCH"))
    results["parity"] = parity

    sqlargs = lambda s: {"sql": s, "output_format": "json"}
    ezargs = lambda s: {"query": s}
    clients = [
        ("sqlhandler-nfs-mcp", sh_nfs, "run_sql", sqlargs, 0),
        ("sqlhandler-s3-mcp", sh_s3, "run_sql", sqlargs, 0),
        ("ezpresto-mcp", ezp, "execute_query", ezargs, 1),
    ]
    bench_legs(clients, results)

    print("BENCH_JSON_BEGIN")
    print(json.dumps(results, indent=1))
    print("BENCH_JSON_END")
    print("COMPACT_BEGIN")
    for leg_name in ("sqlhandler-nfs-mcp", "sqlhandler-s3-mcp", "ezpresto-mcp"):
