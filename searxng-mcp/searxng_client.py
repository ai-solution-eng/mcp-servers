...
_LANGUAGES = {
    ...
    "de", "el", "en", "eo", "es", "et", "eu", "fa", "fi", "fr", "ga", "gd",
    "gl", "gu", "he", "hi", "hr", "ht", "hu", "hy", "id", "is", "it", "ja",
    "ka", "kk", "km", "kn", "ko", "ky", "lo", "lt", "lv", "mg", "mk", "ml",
    "mn", "mr", "ms", "mt", "my", "ne", "nl", "no", "pa", "pl", "ps", "pt",
    "ro", "ru", "sd", "si", "sk", "sl", "so", "sq", "sr", "sv", "sw", "ta",
    "te", "tg", "th", "tl", "tr", "uk", "ur", "uz", "vi", "xh", "yi", "zh",
}
...
class SearXNGClient:
    ...
    async def _request(self, params: dict[str, str]) -> dict | None:
        ...
        try:
            ...
            raise SearXNGError(
                f"could not reach SearXNG at {self.base_url}: {e}"
            ) from e
        ...
        try:
            ...
            raise SearXNGError(
                "SearXNG response was not valid JSON — is 'json' listed in "
        ...
        for i, r in enumerate(data.get("results", [])):
            results.append(
                SearchResult(
                    ...
                    position=i + 1,
                    engines=list(r.get("engines") or [r.get("engine", "")] or []),
...
def format_search_response(resp: SearXNGResponse, max_results: int) -> str:
    ...
    if not results and not resp.answers:
        ...
        if resp.unresponsive_engines:
            ...
            if any("uspended" in r or "too many requests" in r.lower()
                   for _, r in resp.unresponsive_engines):
                msg += (
                    "\nNote: some engines are temporarily suspended/rate-limited; "
                    "retrying in a few minutes may help."
                )
        return msg
