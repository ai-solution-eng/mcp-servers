...
class SavedQueryStore:
    ...
    def _load(self) -> None:
        ...
        other way around)."""
        if self.path is None:  # in-memory store — nothing to load
            return
        try:
    ...
    def _flush(self) -> None:
        ...
        if self.path is None:  # in-memory store — nothing to persist
            return
    ...
    def save(self, name: str, query: str, params: dict | None = None) -> dict:
        ...
        stored_raw = raw if raw != clean else clean
        entry = {
            "name": clean,
            "raw_name": stored_raw,
            "query": query_text,
        ...
        if stored_raw != clean:
            self._aliases[stored_raw] = clean
        if self.path:
            try:
                self._flush()
            except BaseException:
