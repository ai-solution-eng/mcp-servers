"""Tiny, honest self-metrics for prometheus-mcp (Wave-3 C3 — fleet item 11).
...
* ADDITIVE and chart-gated DEFAULT-OFF. Nothing here changes any request's
  ...
  PROMETHEUS_METRICS_ENABLED=true (values: metrics.enabled), so the default
...
logger = logging.getLogger("prometheus-mcp.metrics")

METRIC_PREFIX = "prometheus_mcp"
TOOL_REQUESTS = METRIC_PREFIX + "_tool_requests_total"
TOOL_REQUESTS_HELP = "prometheus-mcp MCP tool calls, by tool and outcome."
...
class Metrics:
    ...
    def __init__(self) -> None:
        if _HAVE_PROMETHEUS_CLIENT:
            ...
            # Exactly one backend is live: (registry, prom) here, or mini below.
            self._registry: CollectorRegistry | None = CollectorRegistry()
            self._prom: _PromCounter | None = _PromCounter(
                TOOL_REQUESTS,
                TOOL_REQUESTS_HELP,
                ("tool", "outcome"),
                registry=self._registry,
            )
            self._mini: _MiniCounter | None = None
        else:
            self._registry = None
            self._prom = None
