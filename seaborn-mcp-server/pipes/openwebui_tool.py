...
<body>
  ...
  <script>
    ...
    // Report height to parent iframe for auto-sizing
    ...
    def _inline(self, msg: str, title: Optional[str]) -> HTMLResponse:
        html_doc = (
            ...
            f"<body style='font-family: sans-serif; padding:12px'>{msg}"
            f"<script>function reportHeight(){{const h=document.documentElement.scrollHeight;"
            f"parent.postMessage({{type:'iframe:height',height:h}},'*');}}"
            f"window.addEventListener('load',reportHeight);"
            f"new ResizeObserver(reportHeight).observe(document.body);</script></body></html>"
        )
        return HTMLResponse(content=html_doc, headers={"Content-Disposition": "inline"})
