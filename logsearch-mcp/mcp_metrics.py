"""Tiny, honest self-metrics for logsearch-mcp (Wave-3 C3 — fleet item 11).
...
* ADDITIVE and chart-gated DEFAULT-OFF. Nothing here changes any request's
  ...
  LOGSEARCH_METRICS_ENABLED=true (values: metrics.enabled), so the default
...
logger = logging.getLogger("logsearch-mcp.metrics")

METRIC_PREFIX = "logsearch_mcp"
TOOL_REQUESTS = METRIC_PREFIX + "_tool_requests_total"
TOOL_REQUESTS_HELP = "logsearch-mcp MCP tool calls, by tool and outcome."
...
class Metrics:
    ...
    # Exactly one backend is live per _HAVE_PROMETHEUS_CLIENT; the other stays None.
    _registry: CollectorRegistry | None
    _prom: _PromCounter | None
    _mini: _MiniCounter | None
