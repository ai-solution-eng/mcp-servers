# logsearch-mcp — values examples (secret-free, hardlinked to GitHub)
...
- **Operators** (plain Helm) can render/eyeball then install:
  `helm template logsearch-mcp helm/ -f helm/values-examples/values.g2.yaml`
  and `helm upgrade --install logsearch-mcp helm/ -n <namespace> -f ...`.
- Values walkthrough (required vs optional, namespace policy, gateway, RBAC
  bootstrap, upgrades):
...
| `values.hosted-trial.yaml` | A customer hosted-trial environment — `${DOMAIN_NAME}` placeholders where the PCAI build resolves them, an explicit (non-empty) namespace allowlist. |
...
Comma-separated keys rotate with zero downtime; one shared Secret covers
every fleet server pointed at it.
